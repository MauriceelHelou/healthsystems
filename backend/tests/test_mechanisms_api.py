...
class TestPathwaySearchEndpoint:
    """Tests for GET /api/mechanisms/search/pathway endpoint."""

    def test_search_pathway_direct_connection(self, client: TestClient, test_db: Session):
        """Test finding direct pathways between nodes."""
        # Create nodes
        nodes = [Node(id=f"node{i}", name=f"Node {i}", node_type="stock", category="test") for i in range(3)]
        test_db.add_all(nodes)
        ...
        mech = Mechanism(
            ...
            direction="positive",
            category="test",
            mechanism_pathway=["direct connection"],
            evidence_quality="A",
            evidence_n_studies=5,
            evidence_primary_citation="Test (2024)",
            description="Test mechanism"
        ...
        # Search pathway
        response = client.get("/api/mechanisms/search/pathway?from_node=node0&to_node=node1")
        assert response.status_code == 200
        data = response.json()
        assert data["from_node"] == "node0"
        assert data["to_node"] == "node1"
        assert data["pathways_found"] >= 1
        assert data["pathways"][0]["pathway_length"] == 1

    def test_search_pathway_two_hops(self, client: TestClient, test_db: Session):
        """Test finding two-hop pathways between nodes."""
        # Create nodes
        nodes = [Node(id=f"node{i}", name=f"Node {i}", node_type="stock", category="test") for i in range(3)]
        test_db.add_all(nodes)

        # Create two-hop pathway
        mech1 = Mechanism(
            id="mech1",
            name="Node 0 -> Node 1",
            from_node_id="node0",
        ...
        # Search pathway
        response = client.get("/api/mechanisms/search/pathway?from_node=node0&to_node=node2")
        assert response.status_code == 200
        data = response.json()
        assert data["pathways_found"] >= 1
        assert data["pathways"][0]["pathway_length"] == 2
