...
test.describe('Accessibility - Keyboard Navigation', () => {
  ...
  test('should support Shift+Tab to navigate backwards', async ({ page }) => {
    ...
    const _forwardText = await forwardElement.textContent();
...
test.describe('Accessibility - Screen Reader Support', () => {
  ...
  test('should have proper heading hierarchy', async ({ page }) => {
    ...
    // Wait for React app to load (header is always present)
...
test.describe('Accessibility - Focus Management', () => {
  test('should restore focus after modal closes', async ({ page }) => {
    ...
    if ((await button.count()) > 0) {
      ...
      // @ts-expect-error - stored for potential future use
      const _initialFocus = await page.locator(':focus').textContent();
  ...
  test('should not lose focus on page navigation', async ({ page }) => {
    ...
    if ((await link.count()) > 0) {
      ...
      const focused = page.locator(':focus');
      const tagName = await focused.evaluate((el) => el.tagName).catch(() => 'NONE' as const);

      // Should have some element focused (either a real element or BODY is acceptable)
      expect(typeof tagName === 'string').toBeTruthy();
    }
  });
});
