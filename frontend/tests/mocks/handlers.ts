...
export const handlers = [
  ...
  http.get(`${API_BASE_URL}/api/mechanisms/search/pathway`, ({ request }) => {
    const url = new URL(request.url);
    const source = url.searchParams.get('from_node');
    const target = url.searchParams.get('to_node');

    let results = mockMechanisms;

    if (source) {
      results = results.filter((m) => m.from_node_id === source);
    }

    if (target) {
      results = results.filter((m) => m.to_node_id === target);
    }
